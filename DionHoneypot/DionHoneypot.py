...
#Client Handle
    #doesn't do backspaces or register movement keys
    #Double check it breaks when \r doesn't show response either
#AI
    #Llama implementation
    #Formatting
    #Handle command ai prompts and return responses
#Testing
    #Multiple threading at once
...
def handle_command(command, channel, current_directory, file_structure, client_ip, current_user):
    #The variables need to be changed and returned when altered by the AI
    #So current directory and file strucurt need to be altered then returned to the main program
    #So the changes are actually kept
    #Similar to how it's done in the gen fake directory
    response = ""
    ...
    match command.split()[0]:
        ...
        case "cd" | "pwd" | "dir":
            #prompt: command, current directory, file structure
                ...
                The command has been identified as a navigation command.
                You are given permission to access the command, the current directory and the file structure uses these to simulate authentic navigation.
        ...
        #File Manipulation
        case "mkdir" | "rm" | "del" | "touch":
            #prompt: command, current directory, file structure
                ai_prompt = f"""
                You are simulating a Linux terminal running on a corporate desktop. 
                Only respond with the raw output **exactly** as the terminal would—no explanations, markdown, or extra comments.
                The command has been identified as a file manipulation command.
                You are given permission to access the command, the current directory and the file structure uses these to simulate authentic file manipulation.
                Please alter and create with the parameters of the current directory and file structure to simulate authentic file manipulation.
                The command entered was: {command}
                The current directory is: {current_directory}
                The file structure is as follows (JSON): 
                {file_structure}
        ...
        case "whoami" | "hostname" | "ipconfig" | "systeminfo":
            ...
            #client_ip is the attack do not return that
                ai_prompt = f"""
                You are simulating a Linux terminal running on a corporate desktop. 
                Only respond with the raw output **exactly** as the terminal would—no explanations, markdown, or extra comments.
                The command has been identified as a system information command.
                You are given permission to access the command, the current directory and the file structure uses these to simulate authentic system information feed back.
                Please return and feedback to the attack information by the system you are simulating using the variables provided.
                The command entered was: {command}
